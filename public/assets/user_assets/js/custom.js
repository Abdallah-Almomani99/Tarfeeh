(function ($) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const ticketInput = document.querySelector(".ticket-count");

        // تقليل عدد التذاكر
        document
            .querySelector(".decrease-ticket")
            .addEventListener("click", function () {
                let currentValue = parseInt(ticketInput.value, 10) || 1;
                currentValue = Math.max(currentValue - 1, 1); // لا تقل القيمة عن 1
                ticketInput.value = currentValue;
            });

        // زيادة عدد التذاكر
        document
            .querySelector(".increase-ticket")
            .addEventListener("click", function () {
                let currentValue = parseInt(ticketInput.value, 10) || 1;
                ticketInput.value = currentValue + 1;
            });

        // إعادة التذاكر إلى القيمة الافتراضية
        document
            .querySelector(".clear-ticket-count")
            .addEventListener("click", function () {
                ticketInput.value = 1;
            });
    });
    ...
    $("#concerts")
        ...
        .on("circle-animation-progress", function (event, progress) {
            $(this)
                .find("strong")
                .html(Math.round(83 * progress) + "<i>%</i>");
        });
    ...
    $("#conference")
        ...
        .on("circle-animation-progress", function (event, progress) {
            $(this)
                .find("strong")
                .html(Math.round(25 * progress) + "<i>%</i>");
        });
    ...
    $("#new_artists")
        ...
        .on("circle-animation-progress", function (event, progress) {
            $(this)
                .find("strong")
                .html(Math.round(82 * progress) + "<i>%</i>");
        });
